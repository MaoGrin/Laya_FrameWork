...
/** 变更监听器：UI 订阅刷新用 */
export type BagListener = (container: Container, op: BagOp, detail?: any) => void;

/** addItem 返回值 */
export interface AddResult {
    success: boolean;   // 是否全部放入
    added: number;      // 实际放入数量
    remain: number;     // 放不下的剩余数量
}
...
export class BagService {
    ...
    static get instance(): BagService {
        if (!this._instance) this._instance = new BagService();
        return this._instance;
    }

    private _listeners: BagListener[] = [];

    /** 订阅变更（UI 刷新 / 存档标记） */
    on(listener: BagListener): void {
        this._listeners.push(listener);
    }

    off(listener: BagListener): void {
        const i = this._listeners.indexOf(listener);
        if (i >= 0) this._listeners.splice(i, 1);
    }

    private emit(container: Container, op: BagOp, detail?: any): void {
        for (const l of this._listeners) l(container, op, detail);
    }
     ...
     * 返回实际放入数量与剩余数量。
     */
    addItem(container: Container, item: ItemStack): AddResult {
        const cfg = Config.getItemCfg(item.itemId);
        if (!cfg) return { success: false, added: 0, remain: item.count };
        // 容器级类型过滤（装备栏只收装备等）
        if (!container.canAccept(cfg.kind)) return { success: false, added: 0, remain: item.count };
        ...
        if (cfg.stackable) {
            for (const slot of container.unlockedSlots) {
                if (!slot.stack || slot.stack.itemId !== item.itemId) continue;
                if (slot.stack.count >= cfg.maxStack) continue;
        ...
        while (remain > 0) {
            ...
            slot.stack = new ItemStack(item.itemId, put, item.uuid, item.attrs);
            remain -= put;
        }
     ...
     * 返回实际移除数量。
     */
    removeItem(container: Container, itemId: number, count: number): number {
        let remain = count;
     ...
     */
    moveItem(container: Container, fromIndex: number, toIndex: number): boolean {
        const from = container.getSlot(fromIndex);
        const to = container.getSlot(toIndex);
        if (!from || !to || from.locked || to.locked || !from.stack) return false;
        ...
        // 同物品且可堆叠 → 合并
        const cfg = Config.getItemCfg(fromStack.itemId);
        if (cfg && cfg.stackable && to.stack.itemId === fromStack.itemId) {
            ...
            if (space > 0) {
                ...
                if (fromStack.count <= 0) from.stack = null;
                container.markChanged();
    ...
    /** 交换两个格子 */
    swapItem(container: Container, a: number, b: number): boolean {
        if (!container.swapSlot(a, b)) return false;
     ...
     */
    sortItem(container: Container): void {
        const slots = container.unlockedSlots;

        const stackables = new Map<number, number>(); // itemId -> 总数量
        const nonStackables: ItemStack[] = [];        // 不可堆叠，保留实例

        for (const s of slots) {
            if (!s.stack) continue;
            const cfg = Config.getItemCfg(s.stack.itemId);
        ...
        for (const [itemId, total] of stackables) {
            const cfg = Config.getItemCfg(itemId)!;
            let n = total;
            while (n > 0) {
                const c = Math.min(n, cfg.maxStack);
                result.push(new ItemStack(itemId, c));
                n -= c;
            }
        }
